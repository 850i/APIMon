...
namespace APIMonLib.Hooks.ntdll.dll {
    public class Hook_ZwWriteFile : AbstractHookDescription {
        protected override Delegate createHookDelegate() {
            return new NtDllSupport.DZwWriteFile(ZwWriteFile_Hooked);
        }

        private const int BUFFER_LIMIT = 100;

        // this is where we are intercepting all file accesses!
        private UInt32 ZwWriteFile_Hooked(IntPtr FileHandle, IntPtr Event, IntPtr ApcRoutine, IntPtr ApcContext, IntPtr IoStatusBlock, IntPtr Buffer, Int32 Length, IntPtr ByteOffset, IntPtr Key) {

            preprocessHook();

            UInt32 result = NtDllSupport.ZwWriteFile(  FileHandle,  Event,  ApcRoutine,  ApcContext,  IoStatusBlock,  Buffer,  Length,   ByteOffset,  Key);
            ...
            if (result == NtDllSupport.STATUS_SUCCESS) {
                ...
                transfer_unit["BytesWritten"] = bytes_read;
